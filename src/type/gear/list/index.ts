import GearItemTypeList from "./item"

class GearItemTypeLists {
    lists: GearItemTypeList[] = []

    constructor() {
        this.lists.push(new GearItemTypeList('helmet', [
            "HEAD_CLOTH_AVALON",
            "HEAD_CLOTH_HELL",
            "HEAD_CLOTH_KEEPER",
            "HEAD_CLOTH_MORGANA",
            "HEAD_CLOTH_ROYAL",
            "HEAD_CLOTH_SET1",
            "HEAD_CLOTH_SET2",
            "HEAD_CLOTH_SET3",
            "HEAD_LEATHER_AVALON",
            "HEAD_LEATHER_HELL",
            "HEAD_LEATHER_MORGANA",
            "HEAD_LEATHER_ROYAL",
            "HEAD_LEATHER_SET1",
            "HEAD_LEATHER_SET2",
            "HEAD_LEATHER_SET3",
            "HEAD_LEATHER_UNDEAD",
            "HEAD_PLATE_AVALON",
            "HEAD_PLATE_HELL",
            "HEAD_PLATE_KEEPER",
            "HEAD_PLATE_ROYAL",
            "HEAD_PLATE_SET1",
            "HEAD_PLATE_SET2",
            "HEAD_PLATE_SET3",
            "HEAD_PLATE_UNDEAD"
        ]))
        this.lists.push(new GearItemTypeList('offhand', [
            "OFF_CENSER_AVALON",
            "OFF_BOOK",
            "OFF_DEMONSKULL_HELL",
            "OFF_TOTEM_KEEPER",
            "OFF_ORB_MORGANA",
            "OFF_SHIELD_AVALON",
            "OFF_SHIELD_HELL",
            "OFF_SPIKEDSHIELD_MORGANA",
            "OFF_SHIELD",
            "OFF_TOWERSHIELD_UNDEAD",
            "OFF_TALISMAN_AVALON",
            "OFF_JESTERCANE_HELL",
            "OFF_HORN_KEEPER",
            "OFF_TORCH",
            "OFF_LAMP_UNDEAD"
        ]))
        this.lists.push(new GearItemTypeList('main hand', [
            "MAIN_ARCANESTAFF",
            "2H_ARCANE_RINGPAIR_AVALON",
            "2H_ENIGMATICSTAFF",
            "2H_ARCANESTAFF",
            "2H_ARCANESTAFF_HELL",
            "2H_ENIGMATICORB_MORGANA",
            "MAIN_ARCANESTAFF_UNDEAD",
            "2H_AXE_AVALON",
            "MAIN_AXE",
            "2H_AXE",
            "2H_HALBERD",
            "2H_SCYTHE_HELL",
            "2H_DUALAXE_KEEPER",
            "2H_HALBERD_MORGANA",
            "2H_BOW_AVALON",
            "2H_BOW",
            "2H_BOW_HELL",
            "2H_BOW_KEEPER",
            "2H_LONGBOW",
            "2H_LONGBOW_UNDEAD",
            "2H_WARBOW",
            "2H_CROSSBOW_CANNON_AVALON",
            "2H_CROSSBOW",
            "2H_DUALCROSSBOW_HELL",
            "2H_CROSSBOWLARGE",
            "2H_CROSSBOWLARGE_MORGANA",
            "MAIN_1HCROSSBOW",
            "2H_REPEATINGCROSSBOW_UNDEAD",
            "MAIN_CURSEDSTAFF_AVALON",
            "MAIN_CURSEDSTAFF",
            "2H_DEMONICSTAFF",
            "2H_CURSEDSTAFF",
            "2H_SKULLORB_HELL",
            "2H_CURSEDSTAFF_MORGANA",
            "MAIN_CURSEDSTAFF_UNDEAD",
            "2H_DAGGER_KATAR_AVALON",
            "2H_CLAWPAIR",
            "MAIN_DAGGER",
            "MAIN_DAGGER_HELL",
            "MAIN_RAPIER_MORGANA",
            "2H_DAGGERPAIR",
            "2H_DUALSICKLE_UNDEAD",
            "2H_FIRE_RINGPAIR_AVALON",
            "MAIN_FIRESTAFF",
            "2H_FIRESTAFF",
            "2H_FIRESTAFF_HELL",
            "2H_INFERNOSTAFF",
            "MAIN_FIRESTAFF_KEEPER",
            "2H_INFERNOSTAFF_MORGANA",
            "MAIN_FROSTSTAFF_AVALON",
            "MAIN_FROSTSTAFF",
            "2H_GLACIALSTAFF",
            "2H_FROSTSTAFF",
            "2H_ICEGAUNTLETS_HELL",
            "MAIN_FROSTSTAFF_KEEPER",
            "2H_ICECRYSTAL_UNDEAD",
            "2H_HAMMER_AVALON",
            "2H_HAMMER",
            "MAIN_HAMMER",
            "2H_DUALHAMMER_HELL",
            "2H_RAM_KEEPER",
            "2H_POLEHAMMER",
            "2H_HAMMER_UNDEAD",
            "MAIN_HOLYSTAFF_AVALON",
            "2H_DIVINESTAFF",
            "2H_HOLYSTAFF",
            "2H_HOLYSTAFF_HELL",
            "MAIN_HOLYSTAFF",
            "MAIN_HOLYSTAFF_MORGANA",
            "2H_HOLYSTAFF_UNDEAD",
            "2H_KNUCKLES_AVALON",
            "2H_KNUCKLES_HELL",
            "2H_KNUCKLES_KEEPER",
            "2H_KNUCKLES_MORGANA",
            "2H_KNUCKLES_SET1",
            "2H_KNUCKLES_SET2",
            "2H_KNUCKLES_SET3",
            "2H_DUALMACE_AVALON",
            "2H_FLAIL",
            "2H_MACE",
            "MAIN_MACE_HELL",
            "MAIN_ROCKMACE_KEEPER",
            "MAIN_MACE",
            "2H_MACE_MORGANA",
            "MAIN_NATURESTAFF_AVALON",
            "2H_NATURESTAFF",
            "2H_NATURESTAFF_HELL",
            "MAIN_NATURESTAFF_KEEPER",
            "2H_NATURESTAFF_KEEPER",
            "MAIN_NATURESTAFF",
            "2H_WILDSTAFF",
            "2H_QUARTERSTAFF_AVALON",
            "2H_DOUBLEBLADEDSTAFF",
            "2H_TWINSCYTHE_HELL",
            "2H_IRONCLADEDSTAFF",
            "2H_ROCKSTAFF_KEEPER",
            "2H_COMBATSTAFF_MORGANA",
            "2H_QUARTERSTAFF",
            "MAIN_SPEAR_LANCE_AVALON",
            "2H_GLAIVE",
            "2H_SPEAR",
            "2H_HARPOON_HELL",
            "MAIN_SPEAR_KEEPER",
            "MAIN_SPEAR",
            "2H_TRIDENT_UNDEAD",
            "2H_CLAYMORE_AVALON",
            "2H_CLAYMORE",
            "2H_DUALSWORD",
            "2H_CLEAVER_HELL",
            "MAIN_SCIMITAR_MORGANA",
            "MAIN_SWORD",
            "2H_DUALSCIMITAR_UNDEAD"           
        ]))
        this.lists.push(new GearItemTypeList('body armour', [
            "ARMOR_CLOTH_AVALON",
            "ARMOR_CLOTH_HELL",
            "ARMOR_CLOTH_KEEPER",
            "ARMOR_CLOTH_MORGANA",
            "ARMOR_CLOTH_ROYAL",
            "ARMOR_CLOTH_SET1",
            "ARMOR_CLOTH_SET2",
            "ARMOR_CLOTH_SET3",
            "ARMOR_LEATHER_AVALON",
            "ARMOR_LEATHER_HELL",
            "ARMOR_LEATHER_MORGANA",
            "ARMOR_LEATHER_ROYAL",
            "ARMOR_LEATHER_SET1",
            "ARMOR_LEATHER_SET2",
            "ARMOR_LEATHER_SET3",
            "ARMOR_LEATHER_UNDEAD",
            "ARMOR_PLATE_AVALON",
            "ARMOR_PLATE_HELL",
            "ARMOR_PLATE_KEEPER",
            "ARMOR_PLATE_ROYAL",
            "ARMOR_PLATE_SET1",
            "ARMOR_PLATE_SET2",
            "ARMOR_PLATE_SET3",
            "ARMOR_PLATE_UNDEAD"     
        ]))
        this.lists.push(new GearItemTypeList('boots', [
            "SHOES_CLOTH_AVALON",
            "SHOES_CLOTH_HELL",
            "SHOES_CLOTH_KEEPER",
            "SHOES_CLOTH_MORGANA",
            "SHOES_CLOTH_ROYAL",
            "SHOES_CLOTH_SET1",
            "SHOES_CLOTH_SET2",
            "SHOES_CLOTH_SET3",
            "SHOES_LEATHER_AVALON",
            "SHOES_LEATHER_HELL",
            "SHOES_LEATHER_MORGANA",
            "SHOES_LEATHER_ROYAL",
            "SHOES_LEATHER_SET1",
            "SHOES_LEATHER_SET2",
            "SHOES_LEATHER_SET3",
            "SHOES_LEATHER_UNDEAD",
            "SHOES_PLATE_AVALON",
            "SHOES_PLATE_HELL",
            "SHOES_PLATE_KEEPER",
            "SHOES_PLATE_ROYAL",
            "SHOES_PLATE_SET1",
            "SHOES_PLATE_SET2",
            "SHOES_PLATE_SET3",
            "SHOES_PLATE_UNDEAD"      
        ]))
        this.lists.push(new GearItemTypeList('cape', [
            "CAPE",
            "CAPEITEM_FW_BRIDGEWATCH",
            "CAPEITEM_FW_FORTSTERLING",
            "CAPEITEM_FW_THETFORD",
            "CAPEITEM_FW_MARTLOCK",
            "CAPEITEM_FW_LYMHURST",
            "CAPEITEM_FW_CAERLEON",
            "CAPEITEM_HERETIC",
            "CAPEITEM_UNDEAD",
            "CAPEITEM_DEMON",
            "CAPEITEM_MORGANA",
            "CAPEITEM_KEEPER",
            "CAPE_ARENA_BANNER"
        ]))
    }
}

export default GearItemTypeLists